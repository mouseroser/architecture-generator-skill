...
# 加载 agent 类型检测函数库
source "$SCRIPT_DIR/lib-agent-types.sh"
...
check_file "shared-context/AGENT-PERSONA-STRATEGY.md" "人格分层策略"
...
if [ -d "$WORKSPACE/agents" ]; then
  for agent_dir in "$WORKSPACE/agents"/*; do
    if [ -d "$agent_dir" ]; then
      ...
      # 使用类型检测函数验证
      validate_agent_files "$agent_dir" || ((WARN++))
    fi
  done
fi
