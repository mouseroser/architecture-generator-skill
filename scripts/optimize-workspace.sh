...
if [ -d "$extensions_dir/lossless-claw" ]; then
    echo "✅ Layer 2B: lossless-claw-enhanced 已安装"
    layer2b_installed=true
else
    echo "❌ Layer 2B: lossless-claw 未安装"
    echo "   💡 安装: https://github.com/win4r/lossless-claw"
fi
echo ""
...
echo "📊 Layer 2B (lossless-claw): $([ "$layer2b_installed" = true ] && echo '✅ 已安装' || echo '❌ 未安装')"
...
if [ "$layer2a_installed" = false ] || [ "$layer2b_installed" = false ]; then
    ...
    if [ "$layer2b_installed" = false ]; then
        echo "   lossless-claw (上下文管理):"
        echo "   - GitHub: https://github.com/win4r/lossless-claw"
        echo "   - 安装命令: openclaw plugins install --link ./lossless-claw"
        echo ""
    fi
fi
